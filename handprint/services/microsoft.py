...
class MicrosoftTR(TextRecognition):
    ...
    def _result_from_api(self, path):
        ...
        response = self._api('post', url, headers, data = image)
        ...
        while poll:
            ...
            wait(2)
            response = self._api('get', poll_url, headers, polling = True)
            if isinstance(response, tuple):
                return response         # If get back a tuple, it's an error.
    ...
    def _api(self, method, url, headers, data = None, polling = False):
        from commonpy.network_utils import net
        from functools import partial

        net_call = partial(net, method, url, headers = headers, polling = polling)
        if method == 'post':
            response, error = net_call(data = data)
        else:
            response, error = net_call()
        ...
        if isinstance(error, NetworkFailure):
            ...
            wait(sleep_time)
            return self._api(method, url, headers, data, polling) # Recurse
        elif error:
